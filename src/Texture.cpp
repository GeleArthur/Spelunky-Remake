...
//
	...
	else // srcRect specified
	{
		...
		textTop = srcRect.top / m_Height;
		textBottom = ( srcRect.top + srcRect.height ) / m_Height;
	...
	if ( !( dstRect.width > 0.001f && dstRect.height > 0.001f ) ) // If no size specified use default size
	{
		...
		vertexBottom = vertexTop + defaultDestHeight;
	}
	else
	{
	...
	glBindTexture( GL_TEXTURE_2D, m_Id );
    RendererHelper::DrawTexture(Rectf{vertexLeft, vertexTop, vertexRight, vertexBottom}, Rectf{textLeft, textTop, textRight, textBottom});
}
