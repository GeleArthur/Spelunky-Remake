...
bool utils::IntersectLineSegments( const Vector2f& p1, const Vector2f& p2, const Vector2f& q1, const Vector2f& q2, float& outLambda1, float& outLambda2, float epsilon )
{
	...
	Vector2f p1p2 = Vector2f::FromTo( p1, p2 );
	Vector2f q1q2 = Vector2f::FromTo( q1, q2 );
	...
	if ( std::abs( denom ) > epsilon )
	{
		...
		Vector2f p1q1 = Vector2f::FromTo( p1, q1 );
...
{
	Vector2f ap = Vector2f::FromTo( a, p );
	Vector2f bp = Vector2f::FromTo( b, p );
...
{
	Vector2f ab = Vector2f::FromTo( a, b );
	Vector2f ap = Vector2f::FromTo( a, p );
	...
	if ( distToA > distAB )
	{
		return Vector2f::FromTo( b, p ).Length( );
	}
