...
void Tile::Draw() const
{
	...
	const Rectf rect = m_Collider.GetRect();
	...
	case TileTypes::ground:
		{
			...
			levelTexture->Draw(position, Rectf{variantX, 64.0f + variantY, 64.0f, 64.0f});
		}
		break;
	case TileTypes::ladderTop:
		levelTexture->Draw(position, Rectf{192.0f, 0.0f, 64.0f, 64.0f});
		break;
	case TileTypes::ladder:
		levelTexture->Draw(position, Rectf{128.0f, 0.0f, 64.0f, 64.0f});
		break;
	case TileTypes::spikes:
		{
			const float variantX = float(m_VariantIndex) * 64.0f;
			levelTexture->Draw(position, Rectf{256 + variantX, 384, 64.0f, 64.0f});
		}
		break;
	case TileTypes::pushBlock:
		// TODO: Replace with object
		levelTexture->Draw(position, Rectf{0.0f, 0.0f, 64.0f, 64.0f});
		break;
	case TileTypes::entrance:
	case TileTypes::exit:
		break;
	case TileTypes::unknown:
		levelTexture->Draw(position, Rectf{15*64,26*64,64,64});
		break;
	}
}
