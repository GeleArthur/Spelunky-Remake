...
void Tile::Draw() const
{
	...
	case TileTypes::ground:
		{
			...
			levelTexture->Draw(m_Collider.GetOrigin(), Rectf{variantX, 64.0f + variantY, 64.0f, 64.0f});
		}
		break;
	case TileTypes::ladderTop:
		levelTexture->Draw(m_Collider.GetOrigin(), Rectf{192.0f, 0.0f, 64.0f, 64.0f});
		break;
	case TileTypes::ladder:
		levelTexture->Draw(m_Collider.GetOrigin(), Rectf{128.0f, 0.0f, 64.0f, 64.0f});
		break;
	case TileTypes::spikes:
		{
			const float variantX = float(m_VariantIndex) * 64.0f;
			levelTexture->Draw(m_Collider.GetOrigin(), Rectf{256 + variantX, 384, 64.0f, 64.0f});
		}
		break;
	case TileTypes::pushBlock:
		// TODO: Replace with object
		levelTexture->Draw(m_Collider.GetOrigin(), Rectf{0.0f, 0.0f, 64.0f, 64.0f});
		break;
	...
	case TileTypes::unknown:
		levelTexture->Draw(m_Collider.GetOrigin(), Rectf{15*64,26*64,64,64});
		break;
	}
}
